# Olá, eu sou o Filipi! 👋

Desenvolvedor de Software em formação com sólida base em lógica de programação e experiência em rotinas administrativas e organização de processos. Experiência prática como assistente geral em cartório, unindo atenção a detalhes, organização documental e suporte operacional à resolução de problemas. Atualmente focado no ecossistema de desenvolvimento e análise de dados.

## 🚀 Sobre mim

* 🎓 **Cursando Análise e Desenvolvimento de Sistemas** na FATESG SENAI (Previsão 11/2027).
* 🎯 **Focado no ecossistema Java** e no desenvolvimento de soluções escaláveis.
* 🌍 **Inglês Avançado** (Certificado EF SET).

## 🛠 Tecnologias e Ferramentas

* **Linguagens:** Java (Swing), SQL, Python, JavaScript, HTML/CSS.
* **Ferramentas:** Git, GitHub, NetBeans, VS Code.
* **Infraestrutura:** Windows Server (AD/GPO), VirtualBox, Cisco Packet Tracer.
* **Dados:** Excel Avançado.

## 📈 Projetos em Destaque

### 🚛 [Projeto Integrador - GYNLOG](https://github.com/filipivg/projeto-integrador-faculdade )
Sistema de Gestão Logística e de Frotas desenvolvido como Projeto Integrador.
* **Diferenciais:** Desenvolvimento Java Swing com padrões POO, persistência híbrida (TXT/Excel) e gestão via metodologias ágeis.
* **Stack:** Java, Swing, Excel, Git.

### 📅 [Personal Agenda System](https://github.com/Nomscodes/agenda-swing-layered-architecture )
Aplicação desktop com foco em padrões de projeto e organização de código.
* **Diferenciais:** Arquitetura estruturada no **Modelo de Visão 4+1**, garantindo separação de responsabilidades.
* **Stack:** Java Swing, Camada de Persistência.

### 💻 [Projeto ABAP](https://github.com/filipivg/projeto-abap )
Repositório dedicado ao desenvolvimento e estudos na linguagem ABAP.
* **Diferenciais:** Foco em lógica de programação e estruturação de processos dentro do ecossistema SAP.
* **Stack:** ABAP.

## 📫 Vamos nos conectar?

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white )](www.linkedin.com/in/filipi-vieira-giaconete)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white )](mailto:filipivieiragiaconete@gmail.com)

📧 **E-mail:** [filipivieiragiaconete@gmail.com](mailto:filipivieiragiaconete@gmail.com)
📄 **Currículo:** [Baixar PDF (Atualizado 2026)](./curriculo_filipi_giaconete_2026.pdf)

## 📊 Estatísticas

![GitHub-Stats](https://github-readme-stats-sigma-five.vercel.app/api?username=filipivg&show_icons=true&theme=dracula )
![Top-Langs](https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=filipivg&layout=compact&theme=dracula )
